...
logPocvan = np.log(pocvan, out=np.zeros_like(pocvan), where=(pocvan!=0))
logPoa = np.log(poa, out=np.zeros_like(poa), where=(poa!=0))
logPoacv = np.log(poacv, out=np.zeros_like(poacv), where=(poacv!=0))
logPoacvan = np.log(poacvan, out=np.zeros_like(poacvan), where=(poacvan!=0))
...
plt.plot(N, pocv, color='C9', linestyle='-', marker='.', label='Polynomial approximation with stratified control variate (approx rate: '+"{:.4f}".format(m)+')')
...
m, b = np.polyfit(logN, logPocvan, 1)
plt.plot(N, pocvan, color='C6', linestyle='-', marker='.', label='Polynomial approximation with antithetic control variate (approx rate: '+"{:.4f}".format(m)+')')
m, b = np.polyfit(logN, logPoacvan, 1)
plt.plot(N, poacvan, color='C4', linestyle='-', marker='.', label='Adaptive polynomial approximation with antithetic control variate (approx rate: '+"{:.4f}".format(m)+')')
